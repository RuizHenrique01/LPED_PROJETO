    ...
    do{
	    ...
	    while(fread(&aluno,sizeof(pessoa),1,A)==1){
	        if(aluno.matricula==z){
				...
				printf("faltas    :");
				textcolor(WHITE);
				printf(" %d", aluno.faltas);
				
				textcolor(LIGHT_CYAN);
				gotoxy(35,23);
				printf("media     :");
				textcolor(WHITE);
				printf(" %.2f", aluno.notas[4]);
				
				textcolor(LIGHT_CYAN);
				gotoxy(35,25);
				printf("parciais  :");
				textcolor(WHITE);
				printf(" %.2f  %.2f  %.2f  %.2f", aluno.notas[0],aluno.notas[1],aluno.notas[2],aluno.notas[3]);
...
void excluir(){
    ...
    rewind(p);
    for(int i=0;i<tam;i++){
    	fread(&ler,sizeof(pessoa),1,p);
        fwrite(&ler,sizeof(pessoa),1,A);
    }
    fclose(A);
    fclose(p);
}
...
void logout(){
	int seta=25,seta2=50,mov,set2=25,set=50,aux=1;
	system("cls");
	do{
	//MENU DE LOGOUT
	gotoxy(27,seta2);
